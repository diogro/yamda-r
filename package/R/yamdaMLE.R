...
#' YamdaMLE(pop, list(modules, modules[,-1]))
YamdaMLE = function(data, hypot_list, nneg = FALSE){
  ...
  for(i in seq_along(hypot_list)){
    ...
    current_model = fitZTransCoefML(data, current_hypot)
    ztrans_coef[[i]] = coef(current_model)
    ...
    stats = rbind(stats, calcModelStatsMLE(current_model, names(hypot_list)[[i]]))
  }
